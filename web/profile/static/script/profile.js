...
let endpoint = 'http://52.202.82.247:5000/api/v1/status/' + user_id
...
$('.start-test-btn').click(function(event){
    ...
    assessmentUrl = `http://52.202.82.247:5000/assessment/${currentAssessment}`
    ...
    makeRequest(endpoint).then(data => {
        ...
        let refreshScoresPromise = new Promise((resolve, reject) => {
            $.ajax({
                type: 'PUT',
                url: 'http://52.202.82.247:5000/api/v1/refresh_user',
...
$('#delete-account-link').click(e => {
    ...
    fetch('http://52.202.82.247:5000/api/v1/delete_user', {
        method: 'DELETE'
    })
    .then(res => { return res.json() } )
    .then(userIs => {
        if (userIs.deleted) {
            window.location.replace('http://52.202.82.247:5000');
        }
    })
})
